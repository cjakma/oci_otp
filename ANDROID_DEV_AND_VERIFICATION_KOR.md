...
- 버전: `versionCode 15` / `versionName 0.7.8`
...
| `EnrollmentActivity.java` | 최초 서버 주소 + account_id + userKey(+확인) 입력, device_id 자동 생성 → 위 enrollment 수행, 실패 시 로컬 롤백 |
...
## 7. 수동 테스트 시나리오
1. 설치 후 첫 실행 → 미등록이므로 EnrollmentActivity 표시 → 서버 주소(`https://...`), account_id, userKey 설정 → 앱이 device_id를 자동 생성해 서버 등록 200 확인.
